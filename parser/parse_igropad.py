...
class Igropad:
    ...
    def update_game(self, game):
        request = requests.get(
            Igropad.URL + "/ru/search",
            params={"q": game.get_name().lower().replace(" ", "+"), "submit": "Найти"},
        )
        # print(request.text)
        soup = BeautifulSoup(request.text, "html.parser")
        try:
            sub_URL = (
                soup.find("div", class_="fields")
                .find("h2", class_="field title f_title")
                .find("a")
                .get("href")
            )
            ...
            game.set_rate(
                round(
                    (rate_ip * rate_kol + game.get_rate() * game.rate_num)
